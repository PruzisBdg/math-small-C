...
PRIVATE S16 getN(S16 x, U8 tblPoints)
{
...
extern S16 RShift16( S32 n );


PUBLIC S16 Interpolate2_U8( U8 CONST *tbl, S16 x, U8 tblPoints ) {

   U8 CONST *yp;    // 'y' table pointer
   ...
   x = ClipInt(x, 0, ((tblPoints - 1) << 8) );     // Clip 'x' to the limits of the table
   yp = tbl + getN(x, tblPoints);                  // Pointer to triad of 'y's to interpolate from

   /* Get (x - x0). x ranges from x0 - x2; hence (x - x0) will range from 0 to 2 * 2^8 (512). */
   dx = x - (getN(x, tblPoints) << 8);
